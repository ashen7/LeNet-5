...
public:
    ...
    bool is_image_matrix() const noexcept {
        if (0 == input_array_.size()
                || 0 != image_input_array_.size()) {
            return true;
        } else if (0 != input_array_.size()
                || 0 == image_input_array_.size()) {
            return false;
        } else {
            LOG(FATAL) << "programming occur error, \
                           conv input array or image input array \
                           not initialize, programm is exiting";
            return false;
        }
    }

public:
    int Initialize(int input_height, int input_width, int channel_number, 
    ...
    int Backward(const Matrix3d& sensitivity_array);
...
private:
    ...
    ImageMatrix3d image_input_array_;        //图像输入数组
    Matrix3d padded_input_array_;            //补0填充后的输入数组
    ImageMatrix3d padded_image_input_array_; //补0填充后的图像输入数组
