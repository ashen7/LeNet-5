 ...
 * 补0填充 步长 学习率
 */
int ConvolutionalLayer::Initialize(int input_height, int input_width, int channel_number, 
    ...
    for (int i = 0; i < filter_number; i++) {
        ...
        filters_[i]->Initialize(filter_height, filter_width, channel_number);
    }
 ...
 * 最后经过ReLu激活函数 得到前向计算的输出结果特征图
 */
int ConvolutionalLayer::Forward(const ImageMatrix3d& input_array) {
    //把输入数组存入成员变量 求激活函数导数时要用到  补0填充的也存入 求梯度时用到做为输入 filter是sensitivity map
    image_input_array_ = input_array;
    if (-1 == ImageMatrix::ZeroPadding(image_input_array_, zero_padding_, padded_image_input_array_)) {
        LOG(ERROR) << "convolutional layer forward failed";
        return -1;
    }
    ...
    //遍历每个filter 在输入图像上进行互相关操作 输出特征图深度和filter个数一致
    for (int i = 0; i < filters_.size(); i++) {
        if (-1 == ImageMatrix::Convolution(padded_image_input_array_, filters_[i]->get_weights_array(), 
                                           output_array_[i], filters_[i]->get_bias(), stride_)) {
            LOG(ERROR) << "convolutional layer forward failed";
            return -1;
        }
...
/*
 * 卷积层的反向计算
 * 1. 反向计算传递给前一层的误差项(敏感图)
 * 2. 利用本层的误差项 计算前一层每个权重的梯度 偏置的梯度
 */
int ConvolutionalLayer::Backward(const Matrix3d& sensitivity_array) {
    if (-1 == CalcBpSensitivityMap(sensitivity_array)) {
        LOG(ERROR) << "convolutional layer backward failed";
        return -1;
    }
 ...
 */
void ConvolutionalLayer::UpdateWeights(double learning_rate, int batch_size) {
    for (auto filter : filters_) {
        filter->UpdateWeights(learning_rate, batch_size);
    }
}
...
int ConvolutionalLayer::CalcBpGradient(const Matrix3d& sensitivity_array) {
    ...
    for (int i = 0; i < filters_.size(); i++) {
        ...
        for (int j = 0; j < channel_number_; j++) {
            if (is_image_matrix()) {
                if (-1 == Matrix::Convolution(padded_image_input_array_[j], expanded_sensitivity_array[i], 
...
int ConvolutionalLayer::CalcBpSensitivityMap(const Matrix3d& sensitivity_array) {
    ...
    if (relu_backward_callback_) {
        if (is_image_matrix()) {
            Activator::ReLuImageBackward3d(image_input_array_, derivative_array);
        } else {
...
void ConvolutionalLayer::Dump() const noexcept {
    //调用每个filter的dump
    LOG(INFO) << "filter个数 深度 高 宽: " << filter_number_ << "*"
              << channel_number_ << "*" << filter_height_ << "*"
              << filter_width_;
    for (auto filter : filters_) {
        filter->Dump();
    }
